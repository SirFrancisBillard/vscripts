...
{
	start = Entities.FindByClassname(start, "*")
...
{
	ent.SetForwardVector(target.GetOrigin() - ent.GetOrigin())
}
...
::ChangeHealth <- function(ent, health)
{
...
::ModelPath <- function(str)
	return "models/" + str + ".mdl"

::PlayerModel <- function(str)
	return "models/player/custom_player/legacy/" + str + ".mdl"
// SCOPE VARIABLES
...
::Distance <- function(a, b)
	return sqrt(DistToSqr(a, b))

::DistToSqr2D <- function(a, b)
{
	local dx = abs(a.x - b.x)
	local dy = abs(a.y - b.y)
	return (dx * dx) + (dy * dy)
...
{
	ChatPrintAll(" " + RED + "shit" + VIOLET + "piss" + LIME + "fuck" + GOLD + "cunt" + DARK_RED + "cock" + MAGENTA + "sucker" + YELLOW + "mother" + GREEN + "fucker" + BLUE + "tits")
}
...
::PrintTable <- function(tab, printfunc = printl, indent = "   ")
{
	foreach (k, v in tab)
	{
		if (typeof v == "table")
		{
			PrintTable(v, printfunc, indent + indent);
		}
		else
		{
			printfunc(k + " = " + v)
...
::CenterPrint <- function(ply, msg)
{
	...
	EntFireHandle(temp_hint, "Kill", "", 5)
}
...
::GetActiveWeaponClass <- function(ply)
{
	...
	if (vm != null)
	{
		return TranslateViewmodelToWeaponClass[vm.GetModelName()]
	}
	else
	{
		return "unknown"
...
::GiveMoney <- function(ply, amt, txt = "")
{
	...
	if (typeof ply == "integer")
	{
		if (ply == T)
		{
			...
			// ChatPrintTeam(T, " \x6 +$" + amt + "\x1: " + txt)
		}
		else if (ply == CT)
		{
			EntFireHandle(__moneyman, "AddTeamMoneyCT")
			// ChatPrintTeam(CT, " \x6 +$" + amt + "\x1: " + txt)
		}
	}
	else
	{
...
::HOOKS <- {
		...
		["__base"] = function(data)
		{
			if (data.userid in ConnectData)
			{
				...
				if (ply.ValidateScriptScope() && !("steamid" in ply.GetScriptScope()))
				{
					...
					if (GetDeveloperLevel() > 0)
					{
						ChatPrintAll(" " + BLUE + ss.name + LIGHT_BLUE + " connected. (" + BLUE + ss.steamid + LIGHT_BLUE + ")")
					}
				}
			}
		}
	...
	["player_use"] = {
		["__base"] = function(data)
		{
			if (::UserIDCapturedPlayer != null && UserIDCapturedPlayer.ValidateScriptScope() && data.entity == 0)
			{
...
::EventFired <- function(ent, data)
{
	...
	if (event in HOOKS)
	{
		...
		// always call base functions first
		//if ("__base" in HOOKS[event])
		//	HOOKS[event]["__base"](data)
...
::GetPlayerFromUserID <- function(id)
{
	...
	if (!id)
	{
		printl("using LocalPlayer fallback for null userid (NOT GOOD)")
		return LocalPlayer()
	}
...
::RETRIEVE_WEAPON_CLASSNAMES <- function()
{
	local done = {}
	foreach (v in TranslateViewmodelToWeaponClass)
	{
		if (!(v in done))
		{
			printl("::" + v.toupper() + " <- \"" + v + "\"")
...
::UserIDThink <- function(ply)
{
	...
	{
		local ss = ply.GetScriptScope()
		if (!("userid" in ss) && !("attemptogenerateuserid" in ss))
		{
			...
			ss.attemptogenerateuserid <- true
