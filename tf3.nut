...
OnPostSpawn <- function()
{
	...
	HookToPlayerDeath(function(ply)
	{
		...
		local wep = null
		while (wep = Entities.FindByClassname(wep, "weapon_deagle"))
			if (wep.GetOwner() == null)
				wep.Destroy()
		if (ply.GetModelName() == "models/player/custom_player/legacy/ctm_heavy.mdl")
		{
...
::OverrideVM <- {
	...
	["models/weapons/v_eq_fraggrenade.mdl"] = "models/chicken/chicken.mdl",
	...
	while (ent = Entities.FindByName(ent, "pickup_*"))
	{
		...
		if (user != null)
		{
			...
			else
			{
				...
				foreach (local target in kill_list)
					target.Destroy()
			...
			EntFireHandle(ent, "addoutput", "targetname " + name, 20)
			EntFireHandle(ent, "enabledraw", "", 20)
		}
	}
...
{
	GiveWeapons(ply, ["item_assaultsuit", "weapon_deagle", "weapon_" + KnifeList[RandomInt(0, KnifeList.len() - 1)]])
	MeleeFixup()
	DispatchParticleEffect("firework_crate_explosion_01", ply.GetOrigin(), ply.GetOrigin())
	ply.EmitSound("Player.Respawn")
	if (RandomInt(1, 20) == 1 && !ChunksterActive)
	{
