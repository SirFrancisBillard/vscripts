...
if (!("TTT_CONFIG" in getroottable()))
{
	...
	ChatPrintAll(" " + GRAY + "Setting up TTT config. You should only see this once.")
	...
	AddConfigValue("DETECTIVE_MIN_PLAYERS", 5, "Default karma.")
	...
	AddConfigValue("TRAITOR_PCT", 0.3, "[0-1] Chance to be selected as a traitor.")

	AddConfigValue("JESTER_MIN_PLAYERS", 4, "Minimum players before a jester.")
...
::RoleHintMe <- function(ply, target = -1)
{
	...
	{
		local spec = Entities.FindByClassnameWithin(ply, "player", ply.GetOrigin(), 4)
...
::RoundWin <- function(role)
{
	...
	{
		case "traitor":
			ChatPrintAll(" " + RED + "TRAITORS WIN!")
			break

		case "jester":
			ChatPrintAll(" " + MAGENTA + "JESTER WINS!")
			break

		case "innocent":
			ChatPrintAll(" " + LIME + "INNOCENTS WIN!")
			break
	}
	...
	{
		local hints = [
			"Traitors can't see the sky in their role box because evil people do not deserve happiness!",
			"The hatch to the reactor can be opened by two people.",
			"The detective is immune to infection, but can still suffocate.",
			"Often times, as traitor, you can get the innocents to do your work for you.",
			"If you're innocent, don't shoot the fucking jester.",
			"It's easy to accidentally kill your traitor buddy. Make sure to leverage it in your favor.",
			"Despite the gun having 8 chambers, your chances of losing Russian Roulette are 1 in 6.",
		]
		ChatPrintAll(" " + GRAY + "TIP: " + hints[RandomInt(0, hints.len() - 1)])
	}
}
...
::PlayerKilledPlayer <- function(victim, killer)
{
	...
	if (vrole == krole || (vrole == DETECTIVE && krole == INNOCENT) || (vrole == INNOCENT && krole == DETECTIVE))
	{
		...
		{
			CenterPrint(killer, "2x Karma penalty for killing known teammate. F" + (RandomInt(1, 8) == 1 ? "ri" : "u") + "ck you.")
			AddKarma(killer, -TTT_KARMA_PENALTY * 2)
		}
		else
			AddKarma(killer, -TTT_KARMA_PENALTY)
	...
	if (vrole == JESTER && krole != TRAITOR)
	{
		...
		{
			ChatPrintAll(" \xeget jested nerd LUL")
...
::EmergencyMeeting <- function(ply)
{
	...
	EntFire("tp_fade", "fade")
	local delay = 25
	ChatPrintAll(" " + BLUE + "An EMERGNECY MEETING has been called!")
	ChatPrintAll(" " + BLUE + "You have " + delay + " seconds to deliberate.")
	ChatPrintAll(" " + BLUE + "Gun damage is reduced by 70%.")
...
::AssignRoles <- function()
{
	...
	{
		local delay = 6 + (2 * ts)
		ChatPrintAll(" " + DARK_RED + "Traitors, meet your allies.")
...
::RoundList <- [
	...
	},
	{
		...
		models = ["tm_professional_var1", "tm_professional_var3", "tm_professional_var4"]
	},
	{
	...
	},
	{
		name = "Toaster Bath",
		weps = [
			["weapon_taser", "weapons/w_eq_taser"],
		],
		models = ["tm_professional_var1", "tm_professional_var3", "tm_professional_var4"]
	},
	{
...
OnPostSpawn <- function()
{
	...
	{
		ChatPrintAll(" " + GRAY + "Waiting for warmup to end...")
	}
	else
	{
		ChatPrintAll("Welcome to " + DARK_RED + "TTT" + WHITE +"! Your role is in the top left of your screen.")
		ChatPrintAll("The " + GREEN + "INNOCENTS" + WHITE + " must stick together, survive, and kill the traitor.")
		ChatPrintAll("The " + RED + "TRAITORS" + WHITE + " have access to special traps, and must kill all the others.")
		ChatPrintAll("The " + BLUE + "DETECTIVE" + WHITE + " is proven innocent, and must help the innocents win.")
		ChatPrintAll("The " + MAGENTA + "JESTER" + WHITE + " cannot shoot, but must convince the innocents of his guilt.")
		ChatPrintAll("Only the " + RED + "TRAITORS" + WHITE + " and " + MAGENTA + "JESTER" + WHITE + " can go through vents.")
	}
	...
	if (CURRENT_ROUND)
	{
		ChatPrintAll("Special round: " + MINT + RoundData.name + WHITE + ".")
		EntFire("pickup_weapon_m4a1_silencer", "kill") // destroy traitor m4a1s
	}
	...
	while (wepspawn = Entities.FindByName(wepspawn, "ttt_weapon_spawn"))
	{
		...
		wep.SetAngles(0, RandomInt(-180, 180), (RandomInt(1, 2) == 1) ? 90 : -90)
...
::KillLights <- function()
{
	EntFire("lights", "turnoff")
	EntFire("elights", "turnon")
...
::FixLights <- function()
{
	EntFire("lights", "turnon")
	EntFire("elights", "turnoff")
...
{
	ChatPrintAll(" " + DARK_RED + "WARNING: Oxygen level low!")
	EntFire("o2_alarm", "playsound")
	EntFire("button_restore_o2", "unlock", "", 5)
	EntFire("o2_hurt", "enable", "", 5)
	::O2_FUCKED <- true
}

::RestoreOxygen <- function()
{
	ChatPrintAll(" " + LIME + "Oxygen level returning to normal...")
...
{
	ChatPrintAll(" " + DARK_RED + "Lockdown procedure initiated!")
...
::SetDetectivePreference <- function(ply, b)
{
	if (ply.ValidateScriptScope())
	{
		...
		if (ss.detpref != b)
		{
			CenterPrint(ply, "Opted " + (b ? "IN to" : "OUT of") + " detective lottery.")
			ss.detpref = b
		}
	}
}

::RoleColor <- function(ply)
{
	switch (ply.GetName())
	{
		case "player_detective":
...
::TraitorTester <- function()
{
	...
	for (local i = 0; i < 2; i++)
	{
		EntFire("tester_sprite" + (i + 1), "addoutput", "rendercolor " + RoleColor(subjects[i]))
...
{
	printl(ply + " - " + ply.GetVelocity().Length())
